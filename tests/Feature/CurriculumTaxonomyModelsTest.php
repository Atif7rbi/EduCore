...
class CurriculumTaxonomyModelsTest extends TestCase
{
    ...
    public function test_curriculum_relationship_mapping(): void
    {
        ...
        $stageCurricula = (new EducationStage)->curricula();
        $this->assertInstanceOf(HasMany::class, $stageCurricula);
        $this->assertSame(
            'education_stage_id',
            $stageCurricula->getForeignKeyName()
        );

        $curriculumStage = (new Curriculum)->educationStage();
        $this->assertInstanceOf(BelongsTo::class, $curriculumStage);
        $this->assertSame(
            'education_stage_id',
            $curriculumStage->getForeignKeyName()
        );
